...
# Канонические литералы Adapty для duration
_CANON_DURS = {
    "weekly": "Weekly",
    "monthly": "Monthly",
    "2 months": "2 months",
...
def normalize_durations(durations: Optional[Union[str, List[str]]]) -> Optional[List[str]]:
    ...
    Принимает канон, синонимы и произвольный регистр.
    """
    ...
    for raw in items:
        ...
        # уже канон (с любым регистром)
        if k in canon_values_lower:
            out.append(canon_values_lower[k])
            continue

        # синоним → ключ канона
        if k in _DUR_SYNONYMS:
            key = _DUR_SYNONYMS[k]
            out.append(_CANON_DURS[key])
            continue

        # прямым ключом к канону (напр. 'monthly')
        if k in _CANON_DURS:
            out.append(_CANON_DURS[k])
            continue
...
def _duration_slug(v: str) -> str:
    """Для имени файла: '2 months' -> '2months', 'Monthly' -> 'monthly'."""
...
def _file_suffix(store: Optional[str], durations: Optional[Union[str, List[str]]]) -> str:
    ...
    if s_norm:
        parts.append(s_norm)
    if d_norm:
        parts.append("+".join(_duration_slug(x) for x in d_norm))
...
def fetch_export(
    ...
    *,
    ...
    s_norm = normalize_store(store)
    if s_norm:
        filters["store"] = [s_norm]  # API ждёт list

    d_norm = normalize_durations(durations)
    if d_norm:
        filters["duration"] = d_norm  # уже список канонических значений
...
def build_country_long(
    ...
    *,
    ...
    # стабильный подсчёт разницы в месяцах
    months = (
