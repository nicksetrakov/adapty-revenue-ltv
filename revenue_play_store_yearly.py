...
# --- OPTIONAL FILTERS ---
STORE: Optional[str] = "play_store"   # None | "app_store" | "play_store"
DURATIONS: Optional[Union[str, List[str]]] = ["Annual"]

# >>> annual forecast
ANNUAL_FORECAST_RATE = 0.20  # 20% ко второму году
ALWAYS_FORECAST_ANNUAL = True  # всегда добавляем прогнозный P1, вне зависимости от факта
...
def _file_suffix(store: Optional[str], durations: Optional[Union[str, List[str]]]) -> str:
    ...
    # >>> annual forecast: можно помечать файлы, но по просьбе не меняем имена
...
# --- размер цикла (в месяцах) из выбранной длительности ---
def cycle_months_from_durations(d_norm: Optional[List[str]]) -> int:
    if not d_norm:
        return 1
    mapping = {"Monthly": 1, "2 months": 2, "3 months": 3, "6 months": 6, "Annual": 12}
...
def parse_export_revenue(
    ...
    *,
    ...
    """Сохраняем помесячные индексы (0-based). Агрегацию по «циклам» делаем ниже."""
...
# -------------------------- Helpers (cycles & forecast) --------------
def _add_cycle_index(df: pd.DataFrame, cycle_months: int) -> pd.DataFrame:
    if cycle_months <= 1:
        df = df.copy(); df["cycle_index"] = df["period_index"]; return df
    df = df.copy(); df["cycle_index"] = (df["period_index"] // cycle_months).astype(int); return df
...
def _annual_selected(d_norm: Optional[List[str]]) -> bool:
    return bool(d_norm) and ("Annual" in d_norm)

def _apply_annual_p1_forecast(periodic: pd.DataFrame, rate: float, round_to: int) -> pd.DataFrame:
    """
    periodic: ['country','cohort','cycle_index','period_revenue'] — суммы по всем аппам (уже по ЦИКЛАМ).
    Всегда добавляем ко всем когортам запись для cycle_index=1: + round(P0*rate).
    Если в данных уже есть P1, будет "факт + прогноз".
    """
    if periodic.empty:
        return periodic
    p0 = (periodic[periodic["cycle_index"] == 0]
          .groupby(["country","cohort"], as_index=False)["period_revenue"].sum()
          .rename(columns={"period_revenue":"p0"}))
    add = p0.assign(cycle_index=1,
                    period_revenue=(p0["p0"] * rate).round(round_to))[["country","cohort","cycle_index","period_revenue"]]
    out = pd.concat([periodic, add], ignore_index=True)
    out = (out.groupby(["country","cohort","cycle_index"], as_index=False)
...
def build_country_tables(
    ...
    *,
    ...
    # >>> annual forecast — всегда добавляем P1 = rate * P0
    if _annual_selected(d_norm) and ALWAYS_FORECAST_ANNUAL:
        periodic = _apply_annual_p1_forecast(periodic, ANNUAL_FORECAST_RATE, round_to)

    # Сколько циклов потенциально у когорты
    limits = _max_cycle_index_per_cohort(df_long, end_date=end_date, cycle_months=cycle_months)
    if _annual_selected(d_norm) and ALWAYS_FORECAST_ANNUAL:
        # минимум до P1, чтобы прогноз JUL тоже попал
        limits["max_cycle"] = limits["max_cycle"].clip(lower=1)
...
def build_country_long(
    ...
    *,
    ...
    # периодные суммы по ЦИКЛАМ
    per = (df_cyc.groupby(["country", "cohort", "cycle_index"], as_index=False)
                 .agg(period_revenue=("period_revenue", "sum")))

    # >>> annual forecast — всегда добавляем P1 = rate * P0
    if _annual_selected(d_norm) and ALWAYS_FORECAST_ANNUAL:
        per = _apply_annual_p1_forecast(per, ANNUAL_FORECAST_RATE, round_to)

    # сколько циклов у когорты (минимум 1, если годовые)
    limits = _max_cycle_index_per_cohort(df_long, end_date=end_date, cycle_months=cycle_months)
    if _annual_selected(d_norm) and ALWAYS_FORECAST_ANNUAL:
        limits["max_cycle"] = limits["max_cycle"].clip(lower=1)
...
def run() -> None:
    ...
    # общий long (с аппами) — помесячный сырой ряд
    ...
    rev_raw, rev_pretty = build_country_tables(df_long, end_date=end_date, cycle_months=cycle_months, d_norm=d_norm)
    ...
    country_long_raw, country_long = build_country_long(df_long, end_date=end_date, cycle_months=cycle_months, d_norm=d_norm)
    ...
    if _annual_selected(d_norm) and ALWAYS_FORECAST_ANNUAL:
        print(f"[Annual] Forecast applied: P1 += {ANNUAL_FORECAST_RATE*100:.0f}% of P0 for every cohort (incl. July).")
    print("Saved:")
